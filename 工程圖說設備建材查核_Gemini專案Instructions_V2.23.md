...
**前版：V2.22**  
**前版 SHA-256：`7a072039213a0a8c5210d0fcdc05e17bb4fdfc11b5c4779a8cc8d5a552298352`**  
**本版核心：同步共通規範 V2.16。Multi-Material Batch Review 新增共享頁面視覺去重：同一 Batch 內多材料共用同一原圖頁時，可只載入該唯一頁一次並逐材料獨立判定；相同清晰裁切可於本輪重用。此變更不降低逐材料原圖確認、不共用技術結論、不新增 R3，也不增加 Gemini 對 Claude Final Audit 的工作。**
...
Reserve／Failover Mode 下，若符合共通規範 V2.16 條件，Gemini 可在**全新對話**中另以 `Gemini-FA` 身分執行 Cold Audit；該角色不是前段 R3。
...
Gemini 不得用「Claude 後面會再查」作為放行理由。R1／R2 發現之來源漏洞，應由 GPT＋Gemini 在前段完成：
- 找到正式來源；或
- 改成透明 S3 計算；或
- 明確降格 S5 暫定猜測；或
...
Gemini 的反腦補複核是為了**減少而非增加 Claude Token**。因此 Gemini 不得：
...
本節依共通規範 V2.16 §18 執行。`Deferred Excel Batch` 是使用者明確啟用之 Excel 封版策略，不是第五種 Mode。
...
依共通規範 V2.16 §23，正式 MD 的 Google Drive 上傳、歸檔、重新讀取與 SHA 核對由 GPT 負責。Gemini 不因本規則增加任何上傳操作、技術回合、來源重查、QC 或 Token 負擔。Gemini 仍只需依既有 R1／必要 R2／Gemini-FA 規則輸出可被 GPT 正式整合與留存的內容。
