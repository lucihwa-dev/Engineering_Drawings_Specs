...
**前版：V3.23**  
**前版 SHA-256：`57e06f678abfcea64366421ad73fcbd36cdeac138c6ef3a9a0c98650c21ff27a`**  
**本版核心：同步共通規範 V2.16。GPT 在多材料 Final Audit 工作包中新增 `Final Audit Visual Index`，將相同原圖頁／證據頁合併成唯一頁索引，標示涉及材料與關鍵區域，讓 Claude-FA 一次載入即可逐材料獨立稽核。新增 Shared-Page／Canonical Crop 去重原則與 Audit Count Integrity；此優化只減少重複工具呼叫，不降低 Claude-FA 制度性必查高風險範圍。**
...
若使用者已明確啟用 `Deferred Excel Batch`，第6節點之 Final Audit 路由不變，但 PASS 後依共通規範 V2.16 §18 改進入 `Technical Freeze Gate`／Batch Manifest，不立即寫 Excel。

若 Gemini R1 已完全收斂，可省略 R2。正常不得開 Gemini R3。

本節描述 Normal Mode 標準路由；Reserve／Failover／Safe Stop 的第6節點替代路由依第十五節與共通規範 V2.16 第二十一節執行。
...
1. 先重新查看指定原圖頁面影像；
2. 以原圖／契約／特訂規範優先；
3. 一個可獨立驗證義務原則上一個必要條件；
4. 整體 CNS 符合性與個別性能若均為明文要求，應保持可獨立追蹤；
5. 確切型號不得跨型號／跨系列／跨證書拼證；
6. 搜尋摘要不得單獨判 P/F；
7. P 與 F 使用相同最低證據強度。
...
批次範圍內，GPT 在每材料結束時執行共通規範 V2.16 §18 `Technical Freeze Gate`：
- 固定唯一最終權威 MD 檔名／SHA；
...
本節依共通規範 V2.16 第二十一節執行，優先於本文件中僅描述 Normal Mode 的舊路由文字。`Deferred Excel Batch` 只改變 Excel 封版時點，不改變本節四模式與 Final Auditor 路由。
