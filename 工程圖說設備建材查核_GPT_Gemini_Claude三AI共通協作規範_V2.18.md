...
**前版：V2.17**  
**前版 SHA-256：`74437f804271fa3dc5dc67b798d59a760f2ec25e35f6dde61caa0887c3043894`**  
**本版核心：最小修正 `GPT-W／ChatGPT Work Excel Execution Authorization` 語意。明確區分「GPT-W Excel Execution Package＝正式機械執行規格」與「User Execution Authorization＝實際修改 Excel 的一次性啟動授權」。Package 本身不等於使用者已授權寫入；GPT-W 在建立新 candidate 前須取得一次明確授權。一旦使用者已明確指定 Execution Base／Current Formal Baseline、Execution Package 與執行範圍，GPT-W 不得再要求第二次『是否開始』確認，應立即依既有 Preflight／安全寫入／recalc／V0～V8／V8-L／Drive Seal 流程執行。本版不新增 Gate 編號、不改 Single-Handoff、Claude-FA、Gemini、Reserve／Failover、GPT-W Executor 邊界、V0～V8 或 Excel 五工作頁／欄位／評分／公式治理。**
