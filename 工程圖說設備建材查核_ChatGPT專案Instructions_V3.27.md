...
**版本：V3.27**  
**日期：2026-09-25**  
**適用母規範：三 AI 共通協作規範 V2.19**  
**前版：V3.26**  
**前版 SHA-256：`38f02381a4e05e29a7620adb5c65ff2a61c716628080dbb7380597f9e5a887c5`**  
**本版核心：同步共通規範 V2.19，新增 `Adjacent Batch Combined Run｜相鄰 Batch 合併執行`。保留既有 5 項 Batch-ID，不重編 Master Register；GPT 允許先分 Segment A／B 各完成 5 項完整 GPT①，再將兩個相鄰 Batch（最多10項）合併成一次 Gemini R1、必要時局部 R2、一次 GPT Combined Final、一次 Claude-FA、一次 GPT-W Execution／Formal Seal。新增 Combined Run Safety Split，若 10 項規模影響來源邊界、Claude Token／工具負荷或 Excel 安全，退回原 5＋5 個別完成。同時修正既有 Claude 角色段落中與共通／Claude Instructions 不一致的 Excel／V0～V8 檢查文字，確保 Combined Run 不把 Excel 執行負擔誤送 Claude。本版不新增 Gate 編號、Batch-ID、AI 角色、Executor、Excel 工作頁／欄位／評分／公式，不追溯重開已開始或已封版 Batch。**
...
若使用者已明確啟用 `Deferred Excel Batch`，第6節點之 Final Audit 路由不變，但 PASS 後依共通規範 V2.19 §18 改進入 `Technical Freeze Gate`／Batch Manifest，不立即寫 Excel。

若 Gemini R1 已完全收斂，可省略 R2。正常不得開 Gemini R3。

本節描述 Normal Mode 標準路由；Reserve／Failover／Safe Stop 的第6節點替代路由依第十五節與共通規範 V2.19 第二十一節執行。
...
### 4.4.2 Adjacent Batch Combined Run｜10項、內部5＋5

GPT 可將兩個相鄰既有 5 項 Batch 組成一次 Combined Run；不建立新 Batch-ID、不重編 Master Register。

例如：
`B06＋B07 Combined Run｜M26～M35`

固定做法：
1. `Segment A＝B06｜M26～M30`：逐材料完成 Related Source Page Discovery、原圖影像、Coverage、NR、Candidate Research／Evidence、P/U/F、RFI／限制競爭、MCR、自檢①。
2. `Segment B＝B07｜M31～M35`：重新執行同一完整程序，不得直接沿用 Segment A 技術判定。
3. A＋B 均完成 GPT①後，才建立一份 Combined Gemini R1 Package，內容按 `Batch → Material-ID → Candidate-ID → NR` 分區。
4. Gemini R1 一次處理 10 項；若僅少數材料未閉合，R2 只送指定 Material-ID＋指定爭點，其他材料不得重跑。
5. GPT 收斂後只建立一份 Combined Final；其中每個原 Batch／Material-ID／Candidate-ID／NR／Evidence／P-U-F／RFI／MCR 仍獨立可追溯。
6. GPT Final 前執行一次 Cross-Case／Cross-Material Evidence Contamination Check，特別檢查 B06 規格誤套 B07、同品牌跨型號、跨系列／跨證書、共用 CNS 但不同適用條件等污染。
7. 送 Claude-FA 前建立去重後 `Final Audit Visual Index`，只保留高風險必要內容與唯一證據索引；不得附入 Gemini 歷史全文或重複市場研究。
8. Claude-FA PASS 後只建立一份 M26～M35 GPT-W Execution Package；使用者一次授權，一個 candidate，一次 V0～V8／V8-L／Drive Seal。
9. Formal Completion 以一份 Combined Completion 封裝，但分別固定兩個原 Batch 的 Formal Completion＝YES。

**Combined Run Safety Split：**
若任一 Segment Coverage 無法閉合、10 項工作包已影響材料邊界、Claude Token／工具負荷顯著增加、或 Excel Preflight 顯示一次寫入風險過高：
`Combined Run Safety Split｜原因`

之後回到兩個原 5 項 Batch 各自依既有流程完成。此不是 Reserve／Failover、不是 Targeted Reopen，不重做已完成且仍有效的材料技術結論。

已進入既有 handoff／Final Audit 或已封版之 Batch，不因本規則追溯重開。
...
Claude-E／Claude-FA 只做最後高風險獨立**技術**審核：
- 原圖重大錯讀
- 必要條件漏／重
- Candidate-ID 混型號
- P/F 證據強度不對稱
...
- RFI 錯誤關閉
- 疑似限制競爭核心重大錯誤
- 自然發現足以改變技術結論之重大 `UNSUPPORTED`

Normal Mode 下不得把以下 Excel 執行責任加入 Claude Final Audit：
- V8 五工作頁／唯一鍵／公式實作
- A/B/C/D/E 機械計算驗證
- 外部活頁簿連結檢查
...
上述全部由 GPT-W／ChatGPT Work Executor 階段負責。
...
`Executor＝GPT-W／ChatGPT Work` 是專案層級的預設 Executor 角色指定，不等於特定 Excel 寫入任務的使用者執行授權。

每次要實際建立新 candidate／修改 workbook 前，GPT-W 仍須取得一次明確 `User Execution Authorization`。Execution Package 本身不得被當成已授權寫入。

一旦使用者已明確指定 Execution Base／Current Formal Excel Baseline、Execution Package 與執行範圍，即完成該次唯一一次啟動確認；GPT-W 不得再要求第二次「是否開始」確認。
...
收到有效 `User Execution Authorization` 後，若前置條件全部成立，GPT-W 固定立即執行：
`取得 sealed baseline raw bytes → 建立新 candidate → 寫入 → recalc → V0～V8 → V8-L → Drive raw-byte SHA → Formal Seal`

不得停在 READY 等待一般性再次確認。
...
GPT-W 應採單一 workbook／穩定鍵定位與小區塊批次寫入。標準逐材料封版一次 1 材料；`Adjacent Batch Combined Run` 已完成同一 Combined Final／Final Audit PASS 時，可依一份 Execution Package 在同一 candidate 一次寫入最多 10 項；Deferred Excel Batch 正式封版時，依固定 Manifest 可在同一 candidate 一次寫入整批材料。V1～V6 優先批次檢查，V7 單獨關閉／重開，V8 最後升版與 SHA，避免逐列逐欄工具呼叫。
...
批次範圍內，GPT 在每材料結束時執行共通規範 V2.19 §18 `Technical Freeze Gate`：
- 固定唯一最終權威 MD 檔名／SHA；
...
## 14.3 Adjacent Batch Combined Run｜GPT Formal Completion 責任

兩個相鄰 Batch Combined Run 成功封版後，GPT 只需建立一份 Combined Formal Completion 報告，但必須分別固定：
- `<Batch A> Formal Completion＝YES`
- `<Batch B> Formal Completion＝YES`
- `Current Formal Excel Baseline＝<新正式版本＋SHA>`

Combined Run 只合併執行，不合併或改名既有 Batch 身分。

若 Final Audit／Preflight 前觸發 Safety Split，後續改回兩個原 Batch 個別完成；不得建立兩個競爭 candidate，也不得把未封版 Combined candidate 當 Current Formal Excel Baseline。

---

# 十五、Claude稀缺資源治理與Failover（V3.9新增）

本節依共通規範 V2.19 第二十一節執行，優先於本文件中僅描述 Normal Mode 的舊路由文字。`Deferred Excel Batch` 只改變 Excel 封版時點，不改變本節四模式與 Final Auditor 路由。
...
- 正式檔名因 `(1)`、`(2)` 等重複尾碼造成引用不唯一。
  - 對 `GPT-W Excel Execution Package`，平台／下載產生之 `(1)`、`(2)` 只屬副本檔名；正式身份、Package 引用與授權指令必須恢復為無尾碼之正式工程檔名，例如 `..._V1(1).md` → `..._V1.md`。
...
Adjacent Batch Combined Run 已完成同一 Combined Final／Claude-FA PASS 時，只建立一份涵蓋兩個原 Batch、最多10項材料的 Execution Package；Package 內仍以 `Batch → Material-ID → Candidate-ID／穩定鍵` 分區，不得形成兩份競爭 candidate。
...
**Authorization 與技術工作包分離：**
User Authorization 只啟動既有 Executor 執行，不得修改任何技術內容。技術內容唯一以：
`GPT Final → Source Grounding → 自檢②③ → Claude-FA PASS → GPT-W Execution Package`
固定之正式工作包為準。
...
- `GPT-W Excel Execution Package` 是 READY 的正式執行規格，不等於使用者已授權修改 Excel；
- GPT-W 在第一次 candidate 寫入前必須取得一次明確授權；
- 使用者若已明確指定 Execution Base／Current Formal Excel Baseline、Execution Package、執行範圍，即視為授權成立；
- 授權成立後不得再詢問「是否開始」「是否確認執行」等第二次確認；
- 已授權後且前置條件 PASS，必須立即進入既有 Execution Capacity Preflight／結構安全檢查與後續完整執行鏈；
- 不得因「Execution Package Status＝READY」而把工作停在 READY。
...
Combined Run 亦同：Executor 階段不得攜帶 10 項原圖研究全文；Claude-FA 已 PASS 後只傳純機械 Payload，以避免把 Combined Run 的技術上下文轉嫁到 GPT-W 或再次消耗 Claude Token。
