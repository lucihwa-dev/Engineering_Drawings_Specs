...
**前版：V2.15**  
**前版 SHA-256：`dd23ee30ef1e0136c8d1ae9d066fb4145b76b4ed603015fe4e14c1dfad4701c5`**  
**本版核心：在 V2.15 Multi-Material Batch Review 上新增 `Shared-Page Visual Audit Deduplication` 與 `Canonical Crop Reuse`。同一 Final Audit Batch 內，多材料共用同一原圖頁／同一證據頁時，Final Auditor 應只載入一次並在同一視覺檢查中逐材料獨立判定；同一頁相同關鍵區域之既有清晰裁切可於本輪 Batch 內重用，不得為形式上的「逐材料獨立」而重複載入／裁切。此為工具與 Token 去重，不是審核範圍縮減：原圖重大錯讀、必要條件、Candidate-ID、P/U/F、RFI、MCR、疑似限制競爭與重大 UNSUPPORTED 等必查項仍完整保留。新增 Final Audit Visual Index 與精確頁數紀錄，避免「宣稱查 7 頁但實列 8 頁」類執行紀錄錯誤。**
...
本次治理升版視為專案層級常態授權；後續每一正常版本不需再逐次請使用者重複授權 GPT-W。只有要**改成其他 Executor**時，才需要使用者另行明確授權。
...
每個材料均須：
- GPT 重新查看指定原圖頁面影像；
- Gemini Round 1 重新查看指定原圖；
- Claude-E 最後審核重新查看 GPT 工作包指定之高風險原圖關鍵頁。
...
1. **GPT①**：原圖＋查證＋初判＋自檢① → GPT→Gemini R1 單檔
2. **Gemini①**：獨立全面複核 → Gemini→GPT R1 單檔
...
GPT 是 Source Grounding Gate 的**第一責任者**。在：
- 送 Gemini R1 前；
- Gemini 回覆後的裁決／收斂；
- 最終技術定稿前；
- 產出標單／估價／單價分析前；
...
Gemini 是 Source Grounding Gate 的**獨立第二防線**。Gemini R1／R2 及 Gemini-FA 必須主動挑出：
- GPT 無法指出來源的數據；
- 規格／CNS／型號由語意猜測補成；
- 把系列級資料當確切型號資料；
- 把市場常識或經驗值偽裝成圖說要求；
...
只有 GPT／Gemini 已完成 Source Grounding Gate 後，工作包才可送入 Final Auditor。
