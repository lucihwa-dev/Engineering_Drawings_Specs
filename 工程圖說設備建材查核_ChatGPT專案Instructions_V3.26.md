...
**版本：V3.26**  
**日期：2026-09-25**  
**適用母規範：三 AI 共通協作規範 V2.18**  
**前版：V3.25**  
**前版 SHA-256：`bfb10c8a2e479c62aaa9dcf85774ff809da40fc18021c45f90fa7e41a9b7c0e8`**  
**本版核心：同步共通規範 V2.18，最小修正 GPT-W／ChatGPT Work Excel Execution Authorization 語意。正式區分 `Execution Package＝已定稿機械執行規格` 與 `User Execution Authorization＝實際建立／修改 Excel candidate 的一次性啟動授權`。Package Alone 不得被當成使用者授權；但一旦使用者已明確指定 Execution Base／Current Formal Baseline、Execution Package 與執行範圍，GPT-W 不得再要求第二次『是否開始』確認，應直接依既有 Preflight、安全寫入、recalc、V0～V8、V8-L、Drive Seal 路由執行。本版不新增 Gate 編號、不改 Gemini V2.24／Claude V2.23、不改 Single-Handoff、Reserve／Failover、Executor 邊界、Excel 架構或技術結論。**
...
若使用者已明確啟用 `Deferred Excel Batch`，第6節點之 Final Audit 路由不變，但 PASS 後依共通規範 V2.18 §18 改進入 `Technical Freeze Gate`／Batch Manifest，不立即寫 Excel。

若 Gemini R1 已完全收斂，可省略 R2。正常不得開 Gemini R3。

本節描述 Normal Mode 標準路由；Reserve／Failover／Safe Stop 的第6節點替代路由依第十五節與共通規範 V2.18 第二十一節執行。
...
批次範圍內，GPT 在每材料結束時執行共通規範 V2.18 §18 `Technical Freeze Gate`：
- 固定唯一最終權威 MD 檔名／SHA；
...
本節依共通規範 V2.18 第二十一節執行，優先於本文件中僅描述 Normal Mode 的舊路由文字。`Deferred Excel Batch` 只改變 Excel 封版時點，不改變本節四模式與 Final Auditor 路由。
