...
**前版：V2.16**  
**前版 SHA-256：`896594210b758a2b29a0c2848356808173ddf6a5644bd94b0e3d6fd231819d64`**  
**本版核心：新增 `Related-Page Source Coverage Gate｜關聯原圖頁覆蓋閘門`。GPT 在定 Necessary Requirements 前須先做 Related Source Page Discovery，將工作包既有頁碼視為起始集合而非完整集合；Gemini R1 必須獨立再做一次頁面發現與覆蓋複核。OCR／PDF文字層／全文搜尋僅可定位頁面，所有命中頁仍須重新查看原始頁面影像並標示 Include／Exclude 與排除理由。`Related-Page Coverage＝PASS` 成為前段閉合條件，未 PASS 不得宣告 R1 Closed。本版不新增 Gemini R3、不增加 Claude Final Audit／QC Token、不改 GPT-W Executor、不改 Excel 五工作頁／欄位／評分／公式治理，並完整保留 Shared-Page Visual Audit Deduplication／Canonical Crop Reuse。**
